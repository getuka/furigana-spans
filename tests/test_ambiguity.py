...
def test_ambiguity_resolution_respects_katakana_output() -> None:
    ...
    assert span.reading == "ニッポンバシ"
    assert token.reading == "ニッポンバシ"
    assert token.metadata["resolved_by"] == "ambiguity_rules"
    assert {candidate.reading for candidate in token.candidates} == {"ニホンバシ", "ニッポンバシ"}
